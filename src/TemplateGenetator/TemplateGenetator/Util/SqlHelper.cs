...
namespace TemplateGenerator.Util
{
    ...
    internal abstract class SqlHelper
    {
        ...
        public SqlHelper()
        {
            try
            {
                connectionString = System.Configuration.ConfigurationManager.AppSettings["DbConnString"];
            }
            catch (Exception)
            {
                connectionString = "";
        ...
        #region 读取第一行第一列数据 + ExecuteScalar方法
        /// <summary>
        /// 返回第一行的第一列
        /// </summary>
        ...
        #region MyRegion 为执行命令准备参数 - PrepareCommand(SqlCommand cmd, SqlConnection conn, SqlTransaction trans, CommandType cmdType, string cmdText, SqlParameter[] cmdParms)
        /// <summary>
        /// 为执行命令准备参数
        /// </summary>
